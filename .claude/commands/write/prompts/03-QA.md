...
## 🚨 STEP 0: VOICE CHECK (BLOCKING - BEFORE ALL OTHER QA)
...
**NEVER:**
- ❌ Skip voice check (Step 0)
- ❌ Skip QA review
- ❌ Proceed with rating < 5
- ❌ Self-approve without external model validation
- ❌ Ignore QA feedback
- ❌ Allow hardcoded counts (e.g., "43 tools", "17 skills", "8 agents")
...
- ❌ Fabricated arXiv paper IDs (e.g., made-up paper numbers)
- ❌ Non-existent CVE numbers
- ❌ Incomplete URLs (just domain, not full article path)
- ❌ Claims that don't match the cited source
- ❌ Outdated sources with newer data available
...
🚨 **NO OVERRIDES ALLOWED WITHOUT ADDRESSING EACH CONCERN**
...
- Soften absolute language ("always" → "often", "cannot" → "typically difficult to")
...
1. "[Original claim]" → "[Revised claim]" (Line X)
2. ...

## Logical Gaps Filled
1. "[Gap identified]" → "[How addressed]" (Line X)
2. ...

## Hallucinations Resolved
1. "[Claim]" → VERIFIED with [source] OR REMOVED
2. ...

## Context Added
1. "[Missing context]" → "[Added where]" (Line X)
2. ...

## Evidence Strengthened
1. "[Weak claim]" → "[How strengthened]" (Line X)
2. ...
```

**⛔ CANNOT PROCEED UNTIL grok-response.md EXISTS AND ALL CONCERNS ADDRESSED**
...
🚨 **If rating < 5: STOP. Revise. Re-run QA. Repeat until 5/5.**
